...
<div class="card mb-3">
    ...
    <div class="card-body">
        ...
        {{  config('fitur_reward') }}
        {{  config('operator_bk') }} --}}
        <form action="{{ url('kejadian_siswa/'.$kejadian_siswa->id) }}" method="post" enctype="multipart/form-data" >
            @csrf
            @method('PATCH')
            <input type="hidden" value="{{ $kejadian_siswa->id }}">
            <div class="form-group">
                ...
                <select placeholder="Pilih Siswa" class="form-control {{ $errors->has('id_siswa') ? 'is-invalid':'' }}" name="id_siswa" id="id_siswa" >
                    @foreach ($siswa_list as $key => $value)
                        <option {{ old("id_siswa", $kejadian_siswa->id_siswa) == $key ? "selected":"" }}  value = "{{ $key }}">{{ $value }}</option>
                    @endforeach
            ...
            <div class="form-group">
                ...
                <select placeholder="Pilih Siswa" class="form-control {{ $errors->has('id_kejadian') ? 'is-invalid':'' }}" name="id_kejadian" id="id_kejadian" >
                    @foreach ($kejadian_list as $key => $value)
                        <option {{ old("id_kejadian", $kejadian_siswa->id_kejadian) == $key ? "selected":"" }}  value = "{{ $key }}">{{ $value }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input type="date" value="{{ old('tanggal_kejadian', $kejadian_siswa->tanggaljam_kejadian->format('Y-m-d')) }}" name="tanggal_kejadian" placeholder="Tanggal Kejadian" class="form-control {{ $errors->has('tanggal_kejadian') ? 'is-invalid':'' }}">
            ...
            <div class="form-group">
                ...
                <input type="time" value="{{ old('jam_kejadian', $kejadian_siswa->tanggaljam_kejadian->format('H:i')) }}" name="jam_kejadian" placeholder="Tanggal Kejadian" class="form-control {{ $errors->has('jam_kejadian') ? 'is-invalid':'' }}">
